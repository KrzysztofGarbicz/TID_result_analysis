...
def filter_duplicate_measurement_records(records: pd.DataFrame) -> pd.DataFrame:
    ...
    Example:
    If TPS2553 SN20 has two measured_rising_v records in the same stage,
    both records are shown. If SN21 has only one such record, it is hidden.
    """
    if records.empty:
        return records
    ...
    available_cols = [col for col in duplicate_group_cols if col in out.columns]
    if not available_cols:
        return out.iloc[0:0].copy()

    out["duplicate_group_count"] = (
        out.groupby(available_cols, dropna=False)["__record_uid"]
        .transform("count")
    )
...
def create_measurement_figure(
    ...
    context_suffix = f" | {selected_context}" if normalize_context_key(selected_context) and normalize_context_key(selected_context) != "ALL" else ""
    ...
    for ax, metric in zip(axes, metrics):
        ...
        for (serial, context_key), grp in grouped:
            ...
            artist = ax.scatter(xs, ys, s=36, label=label, picker=8)
            artist._record_uids = grp["__record_uid"].tolist()  # type: ignore[attr-defined]
            artist._x_values = xs  # type: ignore[attr-defined]
            artist._y_values = ys  # type: ignore[attr-defined]
...
class ReviewApp:
    ...
    def __init__(
        ...
        self.hover_artist: Any = None
        self.hover_annotation: Any = None
        self.hover_uid: Optional[str] = None
    ...
    def _build_ui(self) -> None:
        ...
        self.context_combo = ttk.Combobox(top, textvariable=self.context_var, state="disabled", width=22)
        self.context_combo.pack(side=tk.LEFT, padx=5)
        self.context_combo.bind("<<ComboboxSelected>>", lambda event: self.load_refresh_plot())

        self.duplicates_only_check = ttk.Checkbutton(
            top,
            text="Only duplicated measurements",
            variable=self.show_duplicates_only_var,
            command=self.load_refresh_plot,
        )
    ...
    def on_device_changed(self, event: Any = None) -> None:
        """Handle device combobox change."""
        self._refresh_measurement_options()
        self.load_refresh_plot()

    def on_measurement_changed(self, event: Any = None) -> None:
        """Handle measurement combobox change."""
        self._refresh_context_options()
        self.load_refresh_plot()
    ...
    def load_refresh_plot(self) -> None:
        ...
        if logical_name == "fault_and_latch":
            ...
            if self.show_duplicates_only_var.get():
                self._set_info_text(
                    "fault_and_latch table shows only duplicated measurement records. "
                    "Select a row to inspect and flag."
                )
            else:
                self._set_info_text(
                    "fault_and_latch is displayed as a table. Select a row to inspect and flag."
                )
            return

        fig = create_measurement_figure(records, self.config, device, logical_name, self.flagged_points, selected_context=selected_context)

        canvas = FigureCanvasTkAgg(fig, master=self.figure_frame)
        canvas.draw()
        canvas.mpl_connect("pick_event", self.on_pick_point)
        canvas.mpl_connect("motion_notify_event", self.on_hover_point)
        canvas.mpl_connect("figure_leave_event", self.on_leave_figure)
        ...
        if records.empty:
            if self.show_duplicates_only_var.get():
                self._set_info_text("No duplicated measurement records for the selected filters.")
            else:
                self._set_info_text("No data for the selected measurement.")
        else:
            if self.show_duplicates_only_var.get():
                self._set_info_text(
                    "Showing only duplicated measurement records. "
                    "Kliknij punkt, aby zobaczyć rekord i ewentualnie oznaczyć błędny pomiar."
                )
            else:
                self._set_info_text("Kliknij punkt, aby zobaczyć rekord. Najedź kursorem, aby go podświetlić.")
    ...
    def _record_to_plot_xy(self, record: Dict[str, Any]) -> Optional[Tuple[float, float]]:
        """Convert a record to plotted x/y coordinates."""
        y = safe_float(record.get("value"))
        if y is None:
            return None
        stage_pos = stage_position_map(self.config)
        uid = normalize_text(record.get("__record_uid"))
        x = stage_pos.get(normalize_text(record.get("irradiation_stage")), 0) + deterministic_jitter(uid)
        return x, y

    def _clear_hover(self, redraw: bool = True) -> None:
        """Remove temporary hover highlight."""
        changed = False
        if self.hover_artist is not None:
            try:
        ...
        if self.hover_annotation is not None:
            ...
            changed = True
        self.hover_uid = None
        if changed and redraw and self.current_canvas is not None:
            self.current_canvas.draw_idle()

    def _show_hover_for_uid(self, uid: str) -> None:
        """Show highlight and short tooltip for the hovered point."""
        if uid == self.hover_uid:
            return
        record = self.find_record_by_uid(uid)
        if not record or self.current_canvas is None:
            return
        xy = self._record_to_plot_xy(record)
        if xy is None:
            return
        x, y = xy
        ...
        if self.current_figure is not None:
            for ax in self.current_figure.axes:
                if ax.get_title() == metric:
                    target_ax = ax
                    break
        if target_ax is None and self.current_figure and self.current_figure.axes:
            target_ax = self.current_figure.axes[0]
        if target_ax is None:
            return

        self._clear_hover(redraw=False)
        self.hover_artist = target_ax.scatter([x], [y], s=180, facecolors="none", edgecolors="orange", linewidths=2.0, zorder=20)
        tooltip = (
            f"{normalize_text(record.get('lcl_serial_number'))} | {normalize_text(record.get('irradiation_stage'))}\n"
            f"{metric} = {normalize_text(record.get('value'))} {normalize_text(record.get('unit'))}"
        )
        self.hover_annotation = target_ax.annotate(
            tooltip,
            xy=(x, y),
            xytext=(10, 10),
    ...
    def on_hover_point(self, event: Any) -> None:
        """Highlight point under cursor."""
        if self.current_figure is None or event.inaxes is None:
            self._clear_hover()
            return
        ...
        for ax in self.current_figure.axes:
            for artist in ax.collections:
                ...
                self._show_hover_for_uid(uid)
                return

        self._clear_hover()

    def on_leave_figure(self, event: Any) -> None:
        """Clear hover when cursor leaves figure."""
        self._clear_hover()
